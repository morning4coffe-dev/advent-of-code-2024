    ...
    static void Main(string[] args)
    {
        ...
        foreach (var type in locations)
        {
            ...
            for (int i = 0; i < location.Count; i++)
            {
                ...
                {
                    var distance = (location[j].x - location[i].x, location[j].y - location[i].y);

                    antinodes.Add((location[i].x - distance.Item1, location[i].y - distance.Item2));
                    antinodes.Add((location[j].x + distance.Item1, location[j].y + distance.Item2));
                }
            }
        }

        Console.WriteLine(antinodes.Where(IsWithinBounds).Count());
